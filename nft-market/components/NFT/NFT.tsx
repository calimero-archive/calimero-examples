...
export default function NFT({ approved_accounts_ids, metadata, owner_id, token_id, sale_conditions, walletConnection, sellOrBid }: NFTProps) {
    ...
    return (
        <div className="flex flex-col justify-center bg-white h-fit rounded-xl w-fit">
            <div className="h-3/4 p-2 bg-nh-purple-disabled rounded-t-xl">
                <img 
                    src={metadata.media} 
                    alt="nft" 
                    className="rounded-xl object-cover h-80 w-60"
                />
            </div>
            ...
            <div className="flex flex-col justify-center p-2">
                {sellOrBid ? 
                    <>
                        {!sell && <button className="bg-nh-purple hover:bg-nh-purple-highlight w-full p-2 text-black rounded-xl"
                            onClick={async () => {
                               setSell(true)
                            }}
                        >
                        ...
                        {sell &&
                            <div className="flex flex-col gap-2">
                                <input type="number" placeholder="Enter amount" 
                                    className="p-2 text-center rounded-xl focus:outline-none bg-nh-purple-highlight text-white placeholder-white"
                                    value={price} onChange={handlePrice}
                                >
                                </input>
                                <div className="flex justify-center gap-2">
                                    <button className="bg-nh-purple hover:bg-nh-purple-highlight p-2 w-full text-black rounded-xl"
                                        onClick={async () => {
                ...
                :
                <>
                    {!bid && <button className={`bg-nh-purple hover:bg-nh-purple-highlight w-full p-2 text-black rounded-xl
